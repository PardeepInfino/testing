 ...
 * ID:            %!uniqueid!%
 * Last Modified: 2021-01-19T20:20:27+00:00
 * File:          Model/Import.php
...
class Import extends \Magento\Framework\Model\AbstractModel
{
    ...
    {
        $values = [];
        $values[self::ENTITY_STOCK] = __('Products / Stock');
        return $values;
    }
    ...
    protected function runImport($uploadedFile = false)
    {
        try {
            ...
            $processor = $this->objectManager->create(
                'Xtento\StockImport\Model\Processor\\' . ucfirst($this->getProfile()->getProcessor())
            ...
            $import = $this->objectManager->create(
                'Xtento\StockImport\Model\Import\Iterator\\' . ucfirst($this->getProfile()->getEntity())
            );
    ...
    protected function loadFiles()
    {
        ...
        $this->_eventManager->dispatch('xtento_stockimport_files_load_after', ['transport' => $transportObject]);
    ...
    {
        if (!$this->getTestMode() && $this->sources !== false) {
            foreach ($this->sources as $source) {
                try {
    ...
    protected function beforeImport()
    {
        ...
        // $logEntry->setLogDebugMessages(true); // Enable this to store debug log, for example when debugging cron imports
        $this->setLogEntry($logEntry);
        $this->_registry->unregister('stockimport_log');
        $this->_registry->unregister('stockimport_profile');
        $this->_registry->register('stockimport_log', $logEntry);
        $this->_registry->register('stockimport_profile', $this->getProfile());
        \Xtento\StockImport\Helper\GracefulDie::enable();
    }
    ...
    {
        \Xtento\StockImport\Helper\GracefulDie::disable();
        $this->saveLog();
        $this->_registry->unregister('stockimport_profile');
    ...
    {
        // file_put_contents('/tmp/stockimport_'.time().'.log', $this->_registry->registry('stockimport_log')->getDebugMessages());  // Enable this to store debug log, for example when debugging cron imports. ATTENTION: Also enable in beforeImport()
    ...
    public function errorEmailNotification()
    {
        ...
        if ($this->getLogEntry()->getResult() >= Log::RESULT_WARNING) {
            try {
                ...
                $message->setSubject('Magento Stock Import Module @ ' . $this->request->getServer('SERVER_NAME'));
    ...
    protected function checkStatus()
    {
        if (!$this->moduleHelper->confirmEnabled(true)) {
            throw new LocalizedException(
                __(
                    str_rot13(
                        'Gur Fgbpx Vzcbeg Zbqhyr vf abg ranoyrq. Cyrnfr znxr fher lbh\'er hfvat n inyvq yvprafr xrl naq gung gur zbqhyr unf orra ranoyrq ng Flfgrz > KGRAGB Rkgrafvbaf > Fgbpx Vzcbeg pbasvthengvba.'
                    )
                )
            );
        }
