 ...
 * ID:            %!uniqueid!%
 * Last Modified: 2020-10-06T09:47:02+00:00
 * File:          Model/Processor/Mapping/Fields/Configuration.php
 * Copyright:     Copyright (c) XTENTO GmbH & Co. KG <info@xtento.com> / All rights reserved.
 */

namespace Xtento\StockImport\Model\Processor\Mapping\Fields;

use Xtento\StockImport\Model\Processor\Mapping\AbstractConfiguration;
...
class Configuration extends AbstractConfiguration
{
    ...
    public function handleField($field, $value, $fieldConfiguration)
    {
        ...
        if (isset($fieldConfiguration['calculate'])) {
            if (count($fieldConfiguration['calculate']) > 1) {
                foreach ($fieldConfiguration['calculate'] as $config) {
    ...
    protected function arithmeticOperation($field, $value, $config)
    {
        if (isset($config['@'])) {
            ...
            if (isset($configAttributes['value']) && is_numeric($value)) {
                ...
                if ($configAttributes['operation'] === 'round') {
                    $value = round($value, $operationValue);
                }
                if ($configAttributes['operation'] === 'roundnearest') {
                    // Makes 10.14 => 10.10
                    $value = number_format(round($value, $operationValue - 1), $operationValue, '.', '');
                }
            }
        }
        return $value;
