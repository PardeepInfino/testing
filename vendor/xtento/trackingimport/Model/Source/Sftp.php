 ...
 * ID:            %!uniqueid!%
 * Last Modified: 2022-10-29T19:48:08+00:00
 * File:          Model/Source/Sftp.php
...
class Sftp extends AbstractClass
{
    ...
    public function initConnection()
    {
        ...
        if (class_exists('\phpseclib3\Net\SFTP')) { // Magento 2.4.4
            $this->connection = new \phpseclib3\Net\SFTP($this->getSource()->getHostname(), $this->getSource()->getPort(), $this->getSource()->getTimeout());
            ...
            if ($forceDelete) {
                ...
                if ($this->getSource()->getArchivePath() !== "") {
                    ...
                    foreach ($filesToProcess as $file) {
                        ...
                        $renameResult = false;
                        try {
                            $archiveFilePath = $this->getSource()->getArchivePath() . '/' . $file['filename'];
                            if ($this->connection->file_exists($archiveFilePath)) {
                                @$this->connection->delete($archiveFilePath, false);
                            }
                            $renameResult = $this->connection->rename($file['path'] . '/' . $file['filename'], $archiveFilePath);
                        } catch (\Exception $e) {
                            $warning = '(' . __('Detailed Error') . ': ' . substr($e->getMessage(), 0, strrpos($e->getMessage(), ' in ')) . ')';
                        }
                        if (!$renameResult) {
