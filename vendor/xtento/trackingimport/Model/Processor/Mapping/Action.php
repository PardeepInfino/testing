 ...
 * ID:            %!uniqueid!%
 * Last Modified: 2020-08-19T14:33:57+00:00
 * File:          Model/Processor/Mapping/Action.php
...
class Action extends AbstractMapping
{
    ...
    public function getMappingFields()
    {
        ...
        $importActions = [
            ...
            'creditmemo_offline' => [
                'label' => __('Refund offline (*no* online refund via payment gateway module, if possible)'),
                'default_values' => $this->getDefaultValues('yesno'),
                'default_value' => 0,
                'tooltip' => '',
            ],
            ...
            'advanced_settings' => [
                'label' => __('-- Advanced Actions -- '),
                'disabled' => true,
                'tooltip' => '',
            ],
            'invoice_capture_all_invoices' => [
                'label' => __('Capture payment for all invoices of order'),
                'default_values' => $this->getDefaultValues('yesno'),
                'default_value' => 0,
                'tooltip' => 'If enabled, for all invoices belonging to the order you are processing, that havent been captured yet, the extension will try to capture the payments. Even for invoices not created by our extension.',
            ],
            'invoice_resend_email' => [
                'label' => __('Force (re-)send invoice emails'),
                'default_values' => $this->getDefaultValues('yesno'),
                'default_value' => 0,
                'tooltip' => 'If enabled, for all invoices belonging to the order you are processing, the invoice email(s) will be (re-)sent to the customer. Useful if you are just wanting to have a profile that triggers invoice emails, but invoices have been created before already.',
            ],
            'shipment_resend_email' => [
                'label' => __('Force (re-)send shipment emails'),
                'default_values' => $this->getDefaultValues('yesno'),
                'default_value' => 0,
                'tooltip' => 'If enabled, for all shipments belonging to the order you are processing, the shipment email(s) will be (re-)sent to the customer. Useful if you are just wanting to have a profile that triggers shipment emails, but shipments have been created before already.',
            ],
            'send_emails_asynchronously' => [
                'label' => __('Send invoice/shipment/credit memo emails asynchronously (see help)'),
                'default_values' => $this->getDefaultValues('yesno'),
                'default_value' => 0,
                'tooltip' => 'This only works if asynchronous email sending is enabled in the Magento configuration. ATTENTION: If asynchronous email sending is not enabled in Magento or if your Magento cronjob does not work this will stop emails from being sent.',
            ],
            /*'custom_actions' => [
