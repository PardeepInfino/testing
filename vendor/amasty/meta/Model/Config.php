    ...
    public function getRecursionConfigData($paths, $storeId)
    {
        ...
        foreach ($bestPaths as $bestPath) {
            ...
            foreach ($configs as $config) {
                ...
                if ($clonedConfig->getCategoryId() == \Magento\Catalog\Model\Category::TREE_ROOT_ID) {
                    // Lowest priority for default category
                    $clonedConfig->setOrder(count($bestPath));
                    $result[] = $clonedConfig;
                } elseif (in_array($clonedConfig->getCategoryId(), $bestPath)) {
                    $clonedConfig->setOrder($orders[$clonedConfig->getCategoryId()]);
                    $result[] = $clonedConfig;
                }
            }
        }
