...
use Amasty\Mage245Fix\Model\ConfigProvider;
...
class Mage245Fix implements SearchResultApplierInterface
{
    ...
    public function apply(): void
    {
        ...
        $ids = $this->configProvider->isMagentoChangesDisabled()
            ? []
            : $this->getProductIdsBySaleability();
    ...
    private function categoryProductByCustomSortOrder($categoryId): array
    {
        ...
        $categoryCondition = is_array($categoryId)
            ? $connection->quoteInto('FIND_IN_SET(cat_index.category_id, ?)', implode(',', $categoryId))
        ...
        foreach ($sortOrders as $field => $dir) {
            if ($field === 'name') {
                ...
                $query->joinLeft(
                    ...
                    (SELECT attribute_id FROM {$this->collection->getTable('eav_attribute')}
                ...
                $query->joinLeft(
                    ...
                    . $this->collection->getTable('store') . ' WHERE store_id = ' . $storeId . ')',
