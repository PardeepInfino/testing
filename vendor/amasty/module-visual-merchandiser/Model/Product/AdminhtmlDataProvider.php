...
class AdminhtmlDataProvider extends \Magento\Framework\Model\AbstractModel
{
    ...
    private function getAnyVisibilitySourceCollection(): Collection
    {
        if (!$this->hasData('source_collection_any_visibility')) {
            $collection = clone $this->getSourceCollection();
            $collection->clear();
            $collection->setIsVisibleOnlyFilter(false);
            $this->setData('source_collection_any_visibility', $collection);
        }

        return $this->getData('source_collection_any_visibility');
    }

    private function getSourceCollection(bool $addPriceData = false, bool $emulateArea = true): Collection
    {
        if (!$this->hasData('source_collection')) {
            ...
            if ($this->getRestoreConditions()) {
                /** @var \Amasty\VisualMerch\Model\Rule $rule */
                $rule = $this->initRule();
    ...
    public function getFullPositionDataByStoreId($storeId)
    {
        ...
        $visibleProducts = $sourceCollection->getProductIds();
        $visibleProducts = $this->sortIds($visibleProducts);
        $this->emulation->stopEnvironmentEmulation();

        return array_flip($visibleProducts);
    }
